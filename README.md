...
* rebooting & shutting down router
* opening & closing an ssh tunnel between a service running on router local network and a tunelling service (such as ngrok),
...
Tested with GL firmware v.4.5.17

## Administration

### Conflicting packages

Uninstall the following packages that might disturb telco-vecchio access to router's modem:
* gl-sdk4-ui-sms
* gl-sdk4-sms-forward
* smstools3
...
### Telco-vecchio package wrap up

In addition to daemon binary, Telco-vecchio package contains
* a default configuration file
* init.d scripts 
* luci files for web administration interface

Once daemon is built, a whole package with these additional files can be generated by calling `build-ipkg` script that generates an `ipk` file. that

### Telco-vecchio package installation

The generated `ipk` file can be pushed to the router and packaged can be installed by calling `opkg install` command, resulting in:
* daemon binary placed at `/usr/bin/telco-vecchio`
* configuration file placed at `/etc/telco-vecchio.conf`
* init.d script placed at `/etc/init.d/telco-vecchio`, and service configured to start at boot
* luci main menu file placed at `/usr/share/luci/menu.d/luci-app-telco-vecchio.json`
* luci acl rules file placed at `/usr/share/rpcd/acl.d/luci-app-telco-vecchio.json`
* luci views presenting main actions placed at `/www/luci-static/resources/view/telco-vecchio/actions.js`
* luci views presenting logs placed at `/www/luci-static/resources/view/telco-vecchio/logs.js`
* luci views presenting secondary actions placed at `/www/luci-static/resources/view/telco-vecchio/other.js`

## Telco-vecchio daemon execution

The daemon monitors incoming SMSs and trigger specific commands based on their content.
Logs printed by the daemon are written into two rotating files of the `/tmp/log/telco-vecchio/log` directory 
...
- the status of the tunnelling service configured
- the status of the email service configured
...
This command is triggered by sending to the router an SMS with the following content: `shutdown`

The router first replies to the sender with an SMS indicating that a reboot is going to stop, then stops.

### Opening a tunnel with an application running on router local network
...
For convenience, the tunnel-id can be omitted and thus the daemon closes all the channels open by the user

## Configuration

telco-vecchio daemon runtime behavior is defined from a configuration file having the following parameters.
...
each service is defined with:
    ...
    * an end-point, the path to add to the URL to reach the target application 
...
* tunnel_timeout_sec = 3600
* tunnel_refresh_period_sec = 60
* tunnel_max_number = 1

### Daemon initialization parameters
