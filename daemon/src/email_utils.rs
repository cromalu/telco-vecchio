...
pub struct EmailConfig {
    ...
    pub email_send_timeout_sec: u64,
}
...
pub async fn send_email(config: &EmailConfig, email: &OutgoingEmail) -> common::Result<()> {
    timeout(
        Duration::from_secs(config.email_send_timeout_sec),
        async {
        ...
        .await.unwrap_or_else(|_| {
            error!("send_email: timeout while trying to send email");
            Err(Error::SystemCommandExecutionError)
        })
}